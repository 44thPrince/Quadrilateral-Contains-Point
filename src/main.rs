...
impl State {
    fn check_bounds (&self) -> bool {
        // Derive y = mx + b for points 01, 32 and x = my + b for points 03, 12
        ...
        match &self.point {
            Some(components) => {
                ...
                println!("y lower{:.10}",
                             ((slope_x1*(components[0])) + (self.verticies[0][1] - (slope_x1*self.verticies[0][0]))));
                y_bounds.push((slope_x1*(components[0])) + (self.verticies[0][1] - (slope_x1*self.verticies[0][0])));
                println!("y upper{:.10}",
                             ((slope_x2*(components[0])) + (self.verticies[3][1] - (slope_x2*self.verticies[3][0]))));
                y_bounds.push((slope_x2*(components[0])) + (self.verticies[3][1] - (slope_x2*self.verticies[3][0])));
                println!("x lower{:.10}",
                             ((slope_y1*(components[1])) + (self.verticies[0][0] - (slope_y1*self.verticies[0][1]))));
                x_bounds.push((slope_y1*(components[1])) + (self.verticies[0][0] - (slope_y1*self.verticies[0][1])));
                println!("x upper{:.10}",
                             ((slope_y2*(components[1])) + (self.verticies[1][0] - (slope_y2*self.verticies[1][1]))));
                x_bounds.push((slope_y2*(components[1])) + (self.verticies[1][0] - (slope_y2*self.verticies[1][1])));
    ...
    fn inverse_slope (p1: Vec<f64>, p2: Vec<f64>) -> f64 {
        println!("({:.10} - {:.10}) / ({:.10} - {:.10}) inv slope{:.10}",
               p2[0], p1[0], p2[1], p1[1],
              ((p2[0] - p1[0]) / (p2[1] - p1[1])) as f64);
        return (p2[0] - p1[0]) / (p2[1] - p1[1]) // to calculate slope for y: (x2 - x1) / (y2 - y1)
    }
    fn  slope (p1: Vec<f64>, p2: Vec<f64>) -> f64 {
        println!("slope{:.10}",
              ((p2[1] - p1[1]) / (p2[0] - p1[0])) as f64);
        return (p2[1] - p1[1]) / (p2[0] - p1[0]) // to calculate slope for x: (y2 - y1) / (x2 - x1)
    }
}
fn main() {
    let state = State {
        point: Some(vec![2.5,1.5]),
        verticies: vec![vec![1.0,1.0], vec![3.0,1.0], vec![4.0,3.0], vec![1.0,2.0]]
